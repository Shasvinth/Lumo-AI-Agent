...
def process_combined_query():
    ...
    # If no sources provided, use global selected sources
    if not selected_sources:
        selected_sources = SELECTED_SOURCES
        # If still no sources, use all available
        if not selected_sources and SOURCES:
            selected_sources = list(SOURCES.keys())
    
    # If no websites selected, use all approved websites
    if not selected_websites and APPROVED_WEBSITES:
        selected_websites = list(APPROVED_WEBSITES.keys())
